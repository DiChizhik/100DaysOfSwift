//
//  GameScene.swift
//  Project 23 ninja game
//
//  Created by Diana Chizhik on 22/06/2022.
//
import AVFoundation
import SpriteKit
import SceneKit
...
class GameScene: SKScene {
    ...
    private lazy var activeSliceBG: SKShapeNode = {
        let slice = SKShapeNode()
        slice.zPosition = 2
        slice.strokeColor = UIColor(red: 1, green: 0.9, blue: 0, alpha: 1)
        slice.lineWidth = 9
        return slice
    }()
    
    private lazy var activeSliceFG: SKShapeNode = {
        let slice = SKShapeNode()
        slice.zPosition = 3
        slice.strokeColor = UIColor.white
        slice.lineWidth = 6
        return slice
    }()
    ...
    override func didMove(to view: SKView) {
        ...
        for _ in 0...1000 {
            if let nextSequenceElement = SequenceType.allCases.randomElement() {
                sequence.append(nextSequenceElement)
            }
        }
    ...
    private func createEnemy(forceBomb: ForceBomb = .random) {
        ...
        if enemyType == 0 {
            ...
            if let path = Bundle.main.url(forResource: "sliceBomb", withExtension: "caf") {
